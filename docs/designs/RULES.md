...
- テーブル作成時にアンティを有効にするかどうかをトグルで選択する
- 有効にした場合はアンティ額を指定する（デフォルト: `max(1, big_blind / 10)`）
- 無効の場合はアンティなしとして扱う（API 上は `ante: 0` を送信）
...
- テーブル作成時に `straddle_enabled` を設定することで有効化する
- ストラドルは UTG（BB の左隣）が自発的に BB×2 のブラインドをポストする任意オプション
- 条件: `straddle_enabled = true` かつ `next_hand_straddle = true` かつアクティブプレイヤーが 3 人以上
- ストラドラーのスタックがストラドル額（BB×2）未満の場合はスキップ（ストラドルなしで手が始まる）
- ストラドル時のアクション順: ストラドラー以降が先手、最後にストラドラーが行動する（実質 BB 扱い）
- ストラドル意思はサーバー側の `next_hand_straddle` フィールドで管理する
  - `POST /v1/games/{id}/straddle-intent` でいつでも更新できる
  - `startHand` 実行後に自動リセット（`next_hand_straddle = false`）
  - これにより、どのクライアントが `startHand` を呼んでも正しく反映される
- UI では「次のハンドの UTG になるプレイヤー」（`next_utg_idx`）にのみトグルを表示する
